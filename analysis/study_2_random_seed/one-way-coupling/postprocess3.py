...
def load_avg_ux_from_csv(csv_file):
    ...
    df = pd.read_csv(csv_file,
                     sep=";",
                     header=None,
                     names=["x", "y", "z", "vel_x", "vel_y", "vel_z", "mass"], usecols=range(0,7),
                     )
...
def plot_couette_profile(coupling_cycle, color, ax=plt.gca()):
    ...
    csv_path = Path(f"results/CouetteAvgMultiMDCells_0_0_{coupling_cycle}.csv")
    if not csv_path.exists():
        print(f"File {csv_path} does not exist!", file=sys.stderr)
        return
    data = load_avg_ux_from_csv(csv_path)
    z = np.linspace(0, args.channel_height, num=21)
    y = couette_analytic(
        z, coupling_cycle / 4
...
def parse_args(argv=sys.argv[1:]):
    ...
    # Scenario parameters
    arg_parser.add_argument("--offset", default=5.0)
    arg_parser.add_argument("--wall-velocity", default=0.5, type=float)
    arg_parser.add_argument("--channel-height", default=100.0)
    ...
    arg_parser.add_argument("--coupling-cell-size", default=5.0)
    # Script parameters
    arg_parser.add_argument("--workdir", default=Path(__file__).parent)
    arg_parser.add_argument(
        "--coupling-cycles",
        default="100,400,800,1200,1600,1900",
        type=str,
...
if __name__ == "__main__":
    ...
    if len(coupling_cycles) == 0:
        ...
        for path in Path("./results").glob(f"{filename_prefix}*.csv"):
