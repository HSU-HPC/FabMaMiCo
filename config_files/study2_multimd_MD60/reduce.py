...
def calc_diffs(scenario):
    ...
    df_multimd = get_df_from_filter_csv(filename="0_nofilter.csv")
    
    diff1 = np.square( df_vtk['vel_x_mamico'].to_numpy() - df_multimd['vel_x'].to_numpy()).mean()

    with open("res_multimd.diff", "w") as f:
        f.write(str(diff1))
